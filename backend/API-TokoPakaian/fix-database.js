...
async function fixDatabase() {
  try {
    ...
    // 4. Cek apakah user kosong
    const userCheck = await pool.query('SELECT COUNT(*) FROM users');
    if (parseInt(userCheck.rows[0].count) === 0) {
      console.log('👥 Mengisi data user default (admin & kasir)...');
      const adminPass = await bcrypt.hash('admin123', 12);
      const kasirPass = await bcrypt.hash('kasir123', 12);
      
      await pool.query(
        'INSERT INTO users (name, email, password, role) VALUES ($1, $2, $3, $4)',
        ['Administrator', 'admin@demo.com', adminPass, 'admin']
      );
      await pool.query(
        'INSERT INTO users (name, email, password, role) VALUES ($1, $2, $3, $4)',
        ['Kasir Toko', 'kasir@demo.com', kasirPass, 'kasir']
      );
    }
