...
router.get('/stats', authenticateToken, async (req, res) => {
  try {
    ...
    const transactionStats = await pool.query(
      "SELECT SUM(total_amount) as total_sales, COUNT(*) as new_transactions FROM transactions WHERE status = 'completed'"
    );
    stats.total_sales = parseFloat(transactionStats.rows[0].total_sales || 0);
    stats.new_transactions = parseInt(transactionStats.rows[0].new_transactions || 0);

    // Products Sold
    const productsSold = await pool.query(
      "SELECT SUM(quantity) as products_sold FROM transaction_items"
    );
    stats.products_sold = parseInt(productsSold.rows[0].products_sold || 0);

    // Total Stock & Total Products
    const productStats = await pool.query(
      "SELECT SUM(stock) as total_stock, COUNT(*) as total_products FROM products"
    );
    stats.total_stock = parseInt(productStats.rows[0].total_stock || 0);
    stats.total_products = parseInt(productStats.rows[0].total_products || 0);

    // Total Categories
    const categoryStats = await pool.query("SELECT COUNT(*) as total_categories FROM categories");
    stats.total_categories = parseInt(categoryStats.rows[0].total_categories || 0);

    // Total Users
    const userStats = await pool.query("SELECT COUNT(*) as total_users FROM users");
    stats.total_users = parseInt(userStats.rows[0].total_users || 0);
